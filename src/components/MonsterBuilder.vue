  ...
  <div class="max-w-3xl mx-auto bg-white/80 rounded-lg border-2 border-brown-900 p-6 shadow-lg">
    ...
    <form @submit.prevent="previewMonster" class="space-y-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          <label class="block font-medieval mb-2 text-red-900">Hit Points Formula:</label>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block font-medieval mb-2 text-red-900">Challenge Rating:</label>
          <select 
        ...
        <div>
          <label class="block font-medieval mb-2 text-red-900">XP:</label>
          <input 
            type="number" 
      ...
      <!-- Damage Types -->
      <div class="space-y-6">
        <h3 class="font-medieval text-xl text-red-900">Damage Properties</h3>
        
        <!-- Vulnerabilities -->
        <div class="bg-white/50 p-4 rounded-lg border-2 border-brown-900">
          <label class="block font-medieval mb-2 text-red-900">Vulnerabilities:</label>
          <div class="flex flex-wrap gap-2">
            <div v-for="type in damageTypes" :key="'vuln-'+type" 
        ...
        <!-- Resistances -->
        <div class="bg-white/50 p-4 rounded-lg border-2 border-brown-900">
          <label class="block font-medieval mb-2 text-red-900">Resistances:</label>
          <div class="flex flex-wrap gap-2">
            <div v-for="type in damageTypes" :key="'resist-'+type" 
                 class="flex items-center">
              <input 
                type="checkbox"
                :id="'resist-'+type"
        ...
        <!-- Immunities -->
        <div class="bg-white/50 p-4 rounded-lg border-2 border-brown-900">
          <label class="block font-medieval mb-2 text-red-900">Immunities:</label>
          <div class="flex flex-wrap gap-2">
            <div v-for="type in damageTypes" :key="'immune-'+type" 
                 class="flex items-center">
              <input 
                type="checkbox"
                :id="'immune-'+type"
      ...
      <!-- Speed -->
      <div>
        <h3 class="font-medieval text-xl mb-4 text-red-900">Speed</h3>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div v-for="(value, type) in monster.speed" :key="type">
            <label class="block font-medieval mb-2 text-brown-900 capitalize">{{ type }}:</label>
            <div class="flex">
              <input 
                type="number" 
                v-model.number="monster.speed[type]"
                class="w-full bg-parchment border-2 border-r-0 border-brown-900 rounded-l-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-golden" 
                min="0"
                :placeholder="type === 'walk' ? '30' : '0'"
              />
              <span class="bg-parchment border-2 border-l-0 border-brown-900 rounded-r-lg px-2 py-2 text-brown-900 font-medieval">
                ft
              </span>
            </div>
          </div>
      ...
      <div>
        <div class="flex justify-between items-center mb-2">
          ...
          <button 
            @click="addAction" 
            type="button"
            class="bg-brown-900 text-parchment px-3 py-1 rounded-lg font-medieval text-sm hover:bg-red-900 transition-colors duration-200"
          >
            Add Action
          </button>
        </div>
        <div class="space-y-4">
          <div v-for="(action, index) in monster.actions" :key="index" class="bg-white/50 p-4 rounded-lg border-2 border-brown-900">
            <div class="flex justify-between gap-4 mb-2">
              <input 
                v-model="action.name" 
                placeholder="Action Name"
                class="flex-1 bg-parchment border-2 border-brown-900 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-golden"
              />
              <button 
                @click="removeAction(index)" 
                type="button"
                class="bg-red-900 text-parchment px-3 py-1 rounded-lg font-medieval text-sm hover:bg-brown-900 transition-colors duration-200"
              >
                Remove
              </button>
            </div>
...
function addAction() {
  if (!Array.isArray(monster.value.actions)) {
    monster.value.actions = []
  }
...
// Function to update XP based on Challenge Rating
function updateXP() {
...
function previewMonster() {
  ...
  // Create preview with formatted values
  monsterPreview.value = {
    ...monster.value,
    speed: formattedSpeed,
    hit_points_roll: monster.value.hit_dice ? `${monster.value.hit_points} (${monster.value.hit_dice})` : monster.value.hit_points.toString()
  }
  preview.value = true
}
